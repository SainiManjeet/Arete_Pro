...
public class UploadImagesToAwsService extends Service implements LifecycleObserver {
    ...
    ArrayList<ImageUrlReq> mGetImages;
    //    CreateJobReq mJobReq;
    AmazonS3Client s3Client;
    String LOG_TAG = "UploadImagesToAwsService";
    private String NOTIFICATION_CHANNEL_ID = "CreateJob";
    private int NOTIFICATION_ID = 111;
    String mType;

    public UploadImagesToAwsService() {
    }
    ...
    public int onStartCommand(Intent intent, int flags, int startId) {
        ...
        mGetImages = intent.getParcelableArrayListExtra("mImageList");
        startInForeground();
        new UpdateImage().execute(mGetImages);
    ...
    private void startInForeground() {
        ...
        NotificationCompat.Builder builder = new NotificationCompat.Builder(UploadImagesToAwsService.this, NOTIFICATION_CHANNEL_ID)
            ...
            observer.setTransferListener(new TransferListener() {
                ...
                public void onStateChanged(int id, TransferState state) {
                    if (TransferState.COMPLETED == state) {
                        ...
                        req.imageUrl = imgUrl;
                        mImageList.add(req);
                        if (count == mGetImages.size()) {
                            if (mType.equalsIgnoreCase("PRE_WALK"))
                                mJobSpecViewModel.updateJob(new UpdateJobReq(authToken, jobId, mImageList));
                            else
                                mJobSpecViewModel.updateJob(new UpdateJobReq(mImageList,authToken, jobId));
                            stopForeground(true);
                            stopSelf();
                        }
                        Log.d(LOG_TAG, "Upload Image url is >>>" + observer.getAbsoluteFilePath() + "," + observer.getBucket());
                    }
                }
